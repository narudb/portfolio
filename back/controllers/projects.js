...
const getAllProjects = async (req, res) => {
  try {
    let sqlRequest = `SELECT project.id AS project_id, project_title, SUBSTR(project_description, 1, 100) AS projectDescription, project_link, DATE_FORMAT(project_date, " %W, %d %M %Y" ) AS projectDate, project_img, client_name, technologie_name FROM project JOIN client ON client.id=project.client_id JOIN technologie ON technologie.id=technologie_id ORDER BY project_date DESC`;
